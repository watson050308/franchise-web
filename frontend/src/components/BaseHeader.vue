  ...
  <header class="bg-gradient-to-r from-indigo-500 to-teal-400 text-white py-4 px-4 sm:px-6 lg:px-8">
    <nav class="flex flex-col sm:flex-row justify-between items-center max-w-6xl mx-auto">
      ...
      <div :class="['sm:flex items-center space-y-4 sm:space-y-0 sm:space-x-4 mt-4 sm:mt-0 w-full sm:w-auto', {'hidden': !isMenuOpen, 'flex flex-col sm:flex-row': isMenuOpen}]">
        ...
        <div class="relative flex items-center w-full sm:w-auto" @mouseenter="showSearch = true" @mouseleave="hideSearchIfNotFocused">
          <button @click="toggleSearch" class="text-white hover:opacity-80 transition-opacity duration-300 w-full sm:w-auto flex justify-center items-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
            <span class="ml-2 sm:hidden">搜索</span>
          </button>
          <div 
            v-show="showSearch"
            class="absolute top-full left-0 right-0 sm:right-auto sm:left-auto mt-2 sm:mt-0 sm:relative bg-white rounded-lg shadow-lg overflow-hidden z-50 w-full sm:w-64"
          >
            <input
              type="text"
              v-model="searchQuery"
              ...
              placeholder="搜索..."
              class="w-full px-4 py-2 text-gray-700 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-600"
            />
            <div v-if="showSuggestions && filteredBrands.length > 0" class="max-h-60 overflow-y-auto">
              <ul>
                <li 
